...
/**
 * Generates realistic power usage data for the prewarmed cache demo.
 * Creates 92 days of synthetic data with seasonal patterns, weekday/weekend variance,
 * and injected events to simulate real-world power grid behavior.
 */
function generatePowerData(): PowerDataPoint[] {
  ...
  const events: { day: number; type: "spike" | "drop"; magnitude: number }[] = [
    { day: 12, type: "spike", magnitude: 2.8 },
    { day: 15, type: "drop", magnitude: -2.5 },
    { day: 28, type: "spike", magnitude: 3.2 },
    { day: 35, type: "spike", magnitude: 1.8 },
    { day: 42, type: "drop", magnitude: -1.5 },
    { day: 56, type: "spike", magnitude: 2.2 },
    { day: 67, type: "drop", magnitude: -1.8 },
    { day: 78, type: "spike", magnitude: 2.0 },
  ];
  ...
  for (let day = 0; day < days; day++) {
    ...
    let variance = (Math.random() - 0.5) * 0.6;
...
/**
 * Sample grid events spanning July–September 2024 for the prewarmed cache demo.
 * Demonstrates paginated data that persists to IndexedDB for instant display on return visits.
 */
